...
- 在运行时将 YAML 文件作为模块进行 `import` 和 `require`（包括热重载和 watch 模式支持）
- 使用 Bun 的打包器在前端应用中 `import` 和 `require` YAML 文件

---

## 兼容性

Bun 的 YAML 解析器使用 Rust 编写，通过了官方 YAML 测试套件中超过 90% 的测试，涵盖了绝大多数实际使用场景。我们正努力实现 100% 的规范兼容性。
...
- **标量**：字符串、数字、布尔值、空值
- **集合**：序列（数组）和映射（对象）
- **锚点和别名**：使用 `&` 和 `*` 的可复用节点。别名集合共享标识，并且别名可以引用包含自身的集合，因此 `Bun.YAML.parse` 可以返回循环对象（作为模块导入的 YAML 不能是循环的）
- **标签**：如 `!!str`、`!!int`、`!!float`、`!!bool`、`!!null` 等类型提示
- **多行字符串**：字面量（`|`）和折叠（`>`）标量
- **注释**：使用 `#`
- **指令**：`%YAML` 和 `%TAG`
...
直接将 YAML 文件作为 ES 模块导入。Bun 会解析其内容，并将其同时作为默认导出和命名导出提供：
...
你也可以在 CommonJS 中使用 `require` 导入 YAML 文件：
...
当你使用 `bun --hot` 运行应用程序时，Bun 会检测 YAML 文件的更改，并在不关闭连接的情况下重新加载它们。
...
现在，当你修改 `config.yaml` 时，正在运行的应用程序会获取更改：你可以在开发过程中调整设置或切换功能标志，而无需重启应用程序。
...
一个 YAML 文件可以保存多个环境的配置，并通过锚点共享默认值：
...
当你打包导入 YAML 文件的应用程序时，Bun 会在构建时解析 YAML，并将其作为 JavaScript 模块包含在内：
