...
Bun 为 [Redis Pub/Sub](https://redis.io/docs/latest/develop/pubsub/) 协议提供了原生绑定，该功能在 Bun 1.2.23 中加入。
...
<Note>
订阅会接管 `RedisClient` 连接：已订阅的客户端只能调用
`RedisClient.prototype.subscribe()`。要向 Redis 发送其他命令，请使用 `.duplicate()` 创建单独的连接：
