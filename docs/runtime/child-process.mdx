...
在 Linux 上，传入 `cgroup` 可在[控制组](https://docs.kernel.org/admin-guide/cgroup-v2.html)内启动子进程。子进程在开始执行前就会加入 cgroup，因此配置在其中的限制——内存、pids、CPU——会从第一条指令开始生效，并应用于该子进程随后生成的每个进程。当超过内存限制时，内核会在 cgroup _内部_ OOM-kill 一个进程，而不是从父进程回收内存。
