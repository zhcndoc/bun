...
在使用 `"isolated"` 链接器时，将包安装结果跨项目共享到全局虚拟存储 `<cache>/links/` 中，并将 `node_modules/.bun/<pkg>@<ver>` 链接到其中，而不是将每个包实际物化到项目中。可使在 `rm -rf node_modules` 之后的热安装快一个数量级。默认 `true`。也可通过 `BUN_INSTALL_GLOBAL_STORE` 环境变量设置。
