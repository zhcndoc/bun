...
本页面旨在作为在 JavaScript 中处理二进制数据的入门介绍。Bun 实现了多种用于处理二进制数据的数据类型和工具，其中大多数遵循 Web 标准。任何 Bun 特有的 API 都会在文中明确标注。

下面是一份速查表，同时也作为目录。点击左侧栏中的条目即可跳转到对应章节。
...
直到 2009 年，JavaScript 中还没有语言原生的方式来存储和操作二进制数据。ECMAScript v5 引入了一系列新的机制来解决这个问题。最基础的构件是 `ArrayBuffer`，它是一种在内存中表示字节序列的数据结构。
...
要进行有趣的操作，需要所谓的“视图”——一种类，用于 _包装_ 一个 `ArrayBuffer` 实例，让你可以读取和操作其底层数据。有两种视图：_类型化数组_ 和 `DataView`。
