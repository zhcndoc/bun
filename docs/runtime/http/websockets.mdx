...
下面是一个使用 `Bun.serve` 构建的 WebSocket 服务器示例。在 `fetch` 处理器中，所有传入请求都会被 [升级](https://developer.mozilla.org/en-US/docs/Web/HTTP/Protocol_upgrade_mechanism) 为 WebSocket 连接。socket 处理器在 `websocket` 参数中声明。
