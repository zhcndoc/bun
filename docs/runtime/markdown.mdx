...
| ---------------------- | ------- | ------------------------------------------------------------ |
| `tables`               | `true`  | GFM 表格                                                    |
| `strikethrough`        | `true`  | GFM 删除线（`~~text~~`）                                    |
| `tasklists`             | `true`  | GFM 任务列表（`- [x] item`）                                |
| `autolinks`             | `false` | 启用自动链接——参见[自动链接](#自动链接)                     |
| `headings`              | `false` | 标题 ID 和自动链接——参见[标题 ID](#标题-id)                 |
| `hardSoftBreaks`        | `false` | 将软换行视为硬换行                                           |
| `wikiLinks`             | `false` | 启用 `[[wiki links]]`                                        |
| `underline`             | `false` | 将 `__text__` 渲染为 `<u>` 而不是 `<strong>`                |
| `latexMath`             | `false` | 启用 `$inline$` 和 `$$display$$` 数学公式                  |
| `collapseWhitespace`    | `false` | 合并文本中的空白                                             |
| `permissiveAtxHeaders`  | `false` | 允许 `#` 后没有空格的 ATX 标题                               |
| `noIndentedCodeBlocks`  | `false` | 禁用缩进代码块                                               |
| `noHtmlBlocks`          | `false` | 禁用 HTML 块                                                 |
| `noHtmlSpans`            | `false` | 禁用行内 HTML                                                 |
| `tagFilter`             | `false` | 用于过滤不允许 HTML 标签的 GFM 标签过滤器                   |
...
const result = Bun.markdown.render("1. first\n   1. sub-a\n   2. sub-b\n2. second", {
  listItem: (children, { index, depth, ordered, start }) => {
    ...
    // 深度 0 使用 1. 2. 3.，深度 1 使用 a. b. c.，深度 2 使用 i. ii. iii.
