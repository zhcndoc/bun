...
- 使用 `Bun.JSON5.parse` 和 `Bun.JSON5.stringify` 解析和字符串化 JSON5
- 在运行时将 JSON5 文件作为模块 `import` 和 `require`（包括热重载和监视模式支持）
- 在前端应用中使用 Bun 的打包工具 `import` 和 `require` JSON5 文件

---

## 兼容性

Bun 的 JSON5 解析器使用 Rust 编写，并通过了 [官方 JSON5 测试套件](https://github.com/json5/json5-tests)中的 100% 测试。[翻译后的测试套件](https://github.com/oven-sh/bun/blob/main/test/js/bun/json5/json5-test-suite.test.ts)列出了每个测试用例。
...
- **注释**：单行（`//`）和多行（`/* */`）
- **尾随逗号**：对象和数组中
- **未加引号的键**：任何有效的 ECMAScript 5.1 标识符
- **单引号字符串**：除了双引号字符串之外
- **多行字符串**：使用反斜杠进行换行延续
- **十六进制数**：`0xFF`
- **前导和尾随小数点**：`.5` 和 `5.`
...
也可以在 CommonJS 中使用 `require` 导入 JSON5 文件：
...
当你使用 `bun --hot` 运行应用程序时，Bun 会在 JSON5 文件发生更改时重新加载它们：
...
使用 Bun 进行打包时，导入的 JSON5 文件会在构建时解析，并作为 JavaScript 模块包含在内：
