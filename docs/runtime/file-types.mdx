...
XML 文件可以直接导入。Bun 使用原生 XML 1.0 解析器将其解析为 [`Bun.XML.parse`](/runtime/xml) 的紧凑对象结构：根元素对应一个键，属性对应 `"@name"` 键，重复的子元素对应数组，与属性或子元素相邻的文本对应 `"#text"`，并且所有值都是字符串。
...
打包时，解析后的 XML 会以内联的 JavaScript 对象形式嵌入包中。
...
如果将 `.xml` 文件作为入口点传入，它会被转换为一个 `export default` 解析后对象的 `.js` 模块。
