...
`controller.write()` 返回已写入的字节数。当目标端的内部缓冲区已满时（例如较慢的 HTTP 客户端），它会改为返回一个**负数**。数据块仍然会被接受——负返回值表示需要暂停并等待目标端将数据排空。

要等待排空，请 `await controller.flush(true)`：
...
对于默认（非 `direct`）的 `ReadableStream` 和异步生成器响应体，Bun 会自动应用这种背压——当目标端积压时，生产者会暂停。
