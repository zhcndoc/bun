...
Bun 内置支持解析 [JSONL](https://jsonlines.org/)（换行分隔的 JSON），其中每一行都是一个独立的 JSON 值。解析器使用 JavaScriptCore 的优化 JSON 解析器以 C++ 实现，并支持流式处理。
...
try {
  ...
  console.error(error); // SyntaxError: 解析 JSONL 失败
}
```
