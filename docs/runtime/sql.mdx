...
该接口旨在具备高性能，使用带标签的模板字面量来构建查询，并提供诸如连接池、事务和预处理语句等功能。
...
| BIGINT                                  | string, number or BigInt | 如果值适合 i32/u32 大小，则为 number；否则根据 `bigint` 选项为 string 或 BigInt                 |
| DECIMAL, NUMERIC                        | string                   | 为保留精度                                                                                       |
| FLOAT, DOUBLE                           | number                   |                                                                                                   |
| DATE                                    | Date                     | JavaScript Date 对象                                                                             |
| DATETIME, TIMESTAMP                     | Date                     | 按 UTC 解码（见下文说明）；`0000-00-00` 会变成无效日期                                            |
| TIME                                    | number                   | 总微秒数                                                                                         |
| YEAR                                    | number                   |                                                                                                   |
