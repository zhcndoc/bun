...
Bun 内置了高性能的 [SQLite3](https://www.sqlite.org/) 驱动。要使用它，请从内置的 `bun:sqlite` 模块导入。
...
<Accordion title="什么是 WAL 模式？">
  在 WAL 模式下，对数据库的写入会直接写入一个名为 "WAL 文件"（`-wal`）的独立文件中。同时还会创建一个共享内存索引文件（`-shm`）用于读取协调。WAL 文件之后会合并到主数据库文件中。可以将其视为待写入数据的缓冲区。更多详细信息请参阅 [SQLite 文档](https://www.sqlite.org/wal.html)。
</Accordion>
...
- **macOS**：Bun 使用系统提供的 SQLite，Apple 在编译时启用了持久化 WAL。`-wal` 和 `-shm` 文件在关闭后**会持续存在**。这不是错误 —— 这是 Apple 配置系统 SQLite 的方式。
- **Linux** 和 **Windows**：Bun 静态链接了自己的 SQLite 构建，遵循上游默认设置。当没有其他连接打开时，附属文件通常会在关闭后被**删除**。
