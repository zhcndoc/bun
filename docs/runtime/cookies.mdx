...
Bun 提供了通过 `Bun.Cookie` 和 `Bun.CookieMap` 来处理 HTTP cookies 的原生 API。这些 API 提供了快速的方法，用于在 HTTP 请求和响应中解析、生成以及操作 cookies。
...
使用 `Bun.serve()` 时，不必显式调用此方法。对 `req.cookies` map 的任何修改都会自动应用到响应头。这一方法主要适用于其它 HTTP 服务器实现。
