...
- **跨平台**：在 Windows、Linux 和 macOS 上可用。你无需安装额外依赖即可使用 Bun Shell 替代 `rimraf` 或 `cross-env`。诸如 `ls`、`cd`、`rm` 等常见 shell 命令以原生方式实现。
- **熟悉感**：Bun Shell 是一个 bash 风格的 shell，支持重定向、管道、环境变量等。
- **通配符**：通配符模式原生支持，包括 `**`、`*`、`{expansion}` 等。
- **模板字面量**：模板字面量用于执行 shell 命令，允许插入变量和表达式。
- **安全性**：Bun Shell 默认会对所有字符串进行转义，从而防止 shell 注入攻击。
- **JavaScript 互操作**：将 `Response`、`ArrayBuffer`、`Blob`、`Bun.file(path)` 以及其他 JavaScript 对象用作 stdin、stdout 和 stderr。
- **Shell 脚本**：Bun Shell 可用于运行 shell 脚本（`.bun.sh` 文件）。
- **自定义解释器**：Bun Shell 使用 Zig 编写，并包含其词法解析器、语法解析器和解释器。Bun Shell 是一种小型编程语言。
