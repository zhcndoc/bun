...
- 验证并规范化颜色，以便将其持久化到数据库中（`number` 是最适合数据库的）
- 将颜色转换为不同格式
- 除了许多今天正在使用的那 16 种颜色之外进行更丰富的彩色日志（使用 `ansi` 自动检测终端的颜色支持，或使用 `ansi-16`、`ansi-256`、`ansi-16m` 来指定目标颜色深度）
- 为在 HTML 中注入的 CSS 格式化颜色
- 从 CSS 颜色字符串中获取 `r`、`g`、`b` 和 `a` 颜色组件，作为 JavaScript 对象或数字
