...
[AddressSanitizer](https://en.wikipedia.org/wiki/AddressSanitizer) 有助于查找内存问题，并在 Linux 和 macOS 上 Bun 的调试构建中默认启用。包括 Zig 代码以及所有依赖。它会让 Zig 代码构建时间大约变为 2 倍；如果这阻碍了你的效率，你可以用 `bun run build:debug:noasan`（或在 `scripts/build.ts` 中传入 `--asan=off`）禁用它，但通常我们建议在构建之间将改动批量提交。
