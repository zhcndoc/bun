...
<ParamField path="--catalog" type="string">
  将解析后的版本添加到根目录 <code>package.json</code> 的 catalog 中，并以 <code>catalog:</code> 的形式依赖它；{" "}
  <code>--catalog=NAME</code> 指向 <code>catalogs.NAME</code>
</ParamField>

<ParamField path="--filter" type="string">
  将软件包添加到匹配的工作区，而不是当前软件包。别名：<code>-F</code>
</ParamField>
...
<ParamField path="--backend" type="string" default="clonefile">
  用于安装依赖的平台特定优化方式。可选值为 <code>clonefile</code>、<code>hardlink</code>、{" "}
  <code>symlink</code> 或 <code>copyfile</code>
</ParamField>
