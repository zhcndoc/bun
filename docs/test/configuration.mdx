...
使用 glob 模式从测试发现中完全排除文件和目录。与仅影响覆盖率报告的 `coveragePathIgnorePatterns` 不同，`pathIgnorePatterns` 会阻止匹配的路径被作为测试发现和运行。

当你的项目包含子模块、vendored 代码或其他带有 `*.test.ts` 文件但你不想让 `bun test` 拾取的目录时，这非常有用。
...
匹配模式的目录会在扫描期间被剪枝，因此其内容不会被遍历。这意味着忽略大型目录树是高效的——Bun 不会花费时间读取其中的文件。
...
所有测试的默认重试次数。失败测试将最多重试指定次数。单个测试中 `{ retry: N }` 会覆盖此值。默认 `0`（不重试）。
