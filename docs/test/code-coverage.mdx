...
Bun 的测试运行器支持内置的代码覆盖率报告。使用它来查看你的代码库有多少被测试覆盖，并发现当前覆盖不充分的区域。

## 启用覆盖率

`bun:test` 支持查看哪些代码行被测试覆盖。要使用此功能，请在 CLI 传递 `--coverage`。它会将覆盖率报告打印到控制台：
...
默认情况下，覆盖率报告会包含测试文件，并排除源码映射（sourcemaps）。这通常是你想要的，但也可以在 `bunfig.toml` 中进行其他配置。
...
对于 CI 环境中需要持久化的代码覆盖率报告以及其他工具，你可以通过 CLI 传递 `--coverage-reporter=lcov` 选项，或者在 `bunfig.toml` 中设置 `coverageReporter` 选项。
...
默认情况下，测试文件本身会包含在覆盖率报告中。你可以用以下方式将它们排除：
...
此选项接受 glob 模式，类似于 Jest 的 `collectCoverageFrom` 忽略模式。符合任一模式的文件都会从覆盖率计算和报告中排除，包括文本和 LCOV 输出。
...
在内部情况下，Bun 默认转译所有文件，因此 Bun 会自动生成一个内部源码映射，将你的原始源码行映射到 Bun 的内部表示。如果你想禁用此行为，可以设置 `test.coverageIgnoreSourcemaps` 为 `true`；除非是高级用例，否则很少需要这么做。
...
- **排除** `node_modules` 目录
- **排除** 通过非 JS/TS 加载器加载的文件（例如 `.css`、`.txt`），除非你指定了自定义 JS 加载器
...
如果某些文件未出现在覆盖率报告中，可能是测试期间没有导入这些文件。覆盖率只跟踪实际加载的文件。
