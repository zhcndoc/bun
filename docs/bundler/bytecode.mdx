...
使用 `--bytecode` 标志启用字节码缓存。如果没有指定 `--format`，则默认使用 CommonJS：
...
- `dist/index.js` - 你的打包后的 JavaScript（CommonJS）
...
使用 `--compile` 创建可执行文件时，字节码会被嵌入到二进制文件中。支持 ESM 和 CommonJS 两种格式：
...
ESM 字节码需要使用 `--compile`，因为 Bun 会在编译后的二进制文件中嵌入模块元数据（导入/导出信息）。这些元数据使得 JavaScript 引擎在运行时可以完全跳过解析过程。

如果没有 `--compile`，ESM 字节码仍然需要解析源代码以分析模块依赖关系——这就失去了字节码缓存的意义。
...
如果字节码与当前 Bun 版本不匹配，它会被自动忽略，代码将回退为解析 JavaScript 源码。你的应用仍然可以运行——性能优化会被跳过。
