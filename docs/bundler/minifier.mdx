...
遵循 `/*@__PURE__*/` 标注以进行摇树优化（移除未使用的代码）。
...
为了在压缩标识符的同时保留原始函数名和类名以便调试，请使用 `--keep-names` 标志：
...
`--keep-names` 会在继续压缩标识符的同时，保留函数和类的 `.name` 属性。
...
- **`--minify-syntax`:** 在保留可读标识符以便调试的同时生成更小的输出
- **`--minify-identifiers`:** 最大限度地减小体积（结合 `--keep-names` 以获得更好的堆栈跟踪）
