...
<Note>更多示例和高级用法，请参见[编译时常量指南](/guides/runtime/build-time-constants)。</Note>
...
<Warning>这些标志当前无法用于交叉编译，因为它们依赖 Windows API。</Warning>
...
插件可以完成任意转换：YAML/TOML 编译、SQL 查询内联、生成类型安全 API 客户端或预处理模板。详见[插件文档](/bundler/plugins)。
...
- `--public-path`
- `--target=node`
- `--target=browser`（未使用 HTML 入口点 — 关于带 `.html` 文件的 `--compile --target=browser`，请参见[独立 HTML](/bundler/standalone-html)）
