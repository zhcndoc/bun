...
让我们构建第一个 bundle。你有以下两个文件，它们实现了一个客户端渲染的 React 应用。
...
在这里，`index.tsx` 是应用的“入口点”。通常它是执行某些副作用的脚本，比如启动服务器，或者像这里这样，初始化 React 根节点。由于使用了 TypeScript 和 JSX，代码需要先打包才能发送给浏览器。
...
文件路径到内容的映射，用于内存中打包。允许对不存在磁盘上的虚拟文件进行打包，或覆盖已有文件内容。此选项仅在 JavaScript API 中可用。

文件内容可以是 `string`、`Blob`、`TypedArray` 或 `ArrayBuffer`。

#### 完全内存打包

可通过 `files` 在内存中打包所有代码，无需磁盘文件：
...
内存中指定的文件优先级高于磁盘。可覆盖特定文件，保持其他代码不变：
...
<Card title="browser" icon="globe">
  **默认。** 生成面向浏览器的 bundle。解析导入时优先使用 `"browser"` export 条件。导入内建模块（如 `node:events` 或
  `node:path`）可行，但调用部分函数（如 `fs.readFile`）无效。
</Card>
...
<Card title="node" icon="node">
  生成面向 Node.js 的 bundle。解析导入时优先使用 `"node"` export 条件，输出 `.mjs` 文件。未来将自动为 Bun 全局对象以及
  `bun:*` 内建模块提供 polyfill（尚未实现）。
</Card>
...
将文件扩展名映射到内置 loader 名称的表。使用它自定义某些文件的加载方式。
...
要添加到最终 bundle 末尾的 footer。这可以是许可证注释块或有趣的彩蛋。
...
跳过解析未使用的 barrel 文件子模块（重新导出的索引文件）。当你只从一个大型库中导入少量具名导出时，通常打包工具会解析 barrel 文件重新导出的每个文件。使用 `optimizeImports` 后，只有你实际使用的子模块才会被解析。
...
**插件：** 解析和加载插件在桶优化下可以正常工作。延迟加载的子模块在最终被加载时会经过插件流程。

### metafile

生成构建的结构化元数据。metafile 包含所有输入文件、输出文件、大小、导入和导出信息。用途包括：

- **包分析**：了解包大小组成
- **可视化**：传入如 [esbuild 的 bundle analyzer](https://esbuild.github.io/analyze/) 或其他工具
- **依赖追踪**：查看完整导入图
- **CI 集成**：追踪包大小变化
...
使用 `--metafile-md` 生成一个 markdown metafile，它对 LLM 友好，并且在终端中可读：
...
大多数情况下，不需要显式的 try/catch，因为 Bun 会整洁地打印未捕获的异常。你可以直接对 `Bun.build` 调用使用顶层 await。
