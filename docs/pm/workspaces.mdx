...
Bun 在 `package.json` 中支持 [`workspaces`](https://docs.npmjs.com/cli/v9/using-npm/workspaces?v=true#description)。Workspaces 让你将复杂软件开发为一个由多个独立包组成的 _monorepo_。
...
- **代码可以拆分成逻辑部分。** 如果一个包依赖另一个包，就在 `package.json` 中将其添加为依赖。如果包 `b` 依赖 `a`，那么 `bun install` 会把你本地的 `packages/a` 目录安装到 `node_modules` 中，而不是从 npm 注册表下载。
- **依赖可以去重。** 如果 `a` 和 `b` 共享一个共同依赖，它会被 _提升（hoist）_ 到根目录的 `node_modules` 目录。这能减少重复的磁盘占用，并最大程度减少与同时安装多个版本包相关的“依赖地狱”问题。
- **在多个包中运行脚本。** 你可以使用 [`--filter` 标志](/pm/filter) 在你的工作区中对多个包运行 `package.json` 脚本，或使用 `--workspaces` 在所有工作区中运行脚本。
