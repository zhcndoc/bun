...
默认情况下，存储条目会先被物化到 `<cache>/links/` 中的 [global virtual store](/pm/global-store) 一次，而 `node_modules/.bun/<pkg>@<ver>` 则是指向它的符号链接。在 `rm -rf node_modules` 之后进行的热安装，只需为每个包创建一个符号链接，而不必重新复制每个包的文件，在典型的中型项目中大约可快 **7×**。有关完整布局、基准测试和取舍，请参阅 [global store 文档](/pm/global-store)。

### 后端策略

当某个条目不符合 global store 条件（或者 global store 被禁用）时，Bun 会使用以下方式之一将其物化到项目中：
