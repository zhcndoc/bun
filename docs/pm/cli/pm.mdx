...
仅打印受信任的依赖项（允许运行生命周期脚本的依赖）。当在 `package.json` 中设置了 `trustedDependencies` 时，会显示该列表中的包；否则会显示 Bun 的 [默认受信任依赖](https://github.com/oven-sh/bun/blob/main/src/install/default-trusted-dependencies.txt) 列表中的包。
