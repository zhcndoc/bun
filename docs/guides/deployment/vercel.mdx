...
[Vercel](https://vercel.com/) 是一个用于构建、部署和扩展应用的云平台。

<Warning>
  Bun 运行时处于 Beta 阶段。部分功能尚不受支持——例如，自动源映射、字节码缓存，以及 `node:http/https` 的指标。
</Warning>

<Note>
  Vercel Functions 不支持 `Bun.serve`。请将 Bun 与 Vercel 支持的框架结合使用，例如 Next.js、Express、
  Hono 或 Nitro。
</Note>
...
<Steps>
	<Step title="在 vercel.json 中配置 Bun">
    	...
    	Vercel 会自动检测此配置，并使用 Bun 运行你的应用。该值必须为 `"1.x"`；Vercel 会在内部处理次版本号。
    ...
    <Step title="Next.js 配置">
    	...
    	<Note>
    		`--bun` 标志会在 Bun 下运行 Next.js CLI。打包过程（使用 Turbopack 或 Webpack）保持不变。
    	</Note>

    	使用这些脚本后，本地开发和构建都会使用 Bun。
    </Step>
