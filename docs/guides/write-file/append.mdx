...
`fs.appendFile` 异步地向文件追加数据，如果文件尚不存在，则创建该文件。内容可以是字符串或 `Buffer`。
...
请参阅 [Node.js 文档](https://nodejs.org/api/fs.html#fspromisesappendfilepath-data-options) 以了解 `fs.appendFile`。
