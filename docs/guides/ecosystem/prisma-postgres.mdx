...
<Steps>
    ...
    <Step title="用 PostgreSQL 初始化 Prisma">
    	...
    	这会创建一个基础 schema。我们需要将其更新为在 Bun 优化下使用新的无 Rust 客户端。打开 `prisma/schema.prisma` 并修改 generator 块，然后添加一个 `User` 模型。
    ...
    <Step title="创建一个测试脚本">
    	让我们编写一个脚本：创建一个新用户，然后统计数据库中的用户数量。
    ...
    <Step title="运行并测试应用">
    	使用 `bun run` 运行此脚本。每次运行都会创建一个新用户。
