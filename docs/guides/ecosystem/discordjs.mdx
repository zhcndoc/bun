...
你的机器人需要有自己的账号，你可以在 Discord 的开发者门户中创建它。打开[开发者门户](https://discord.com/developers/applications)，登录并创建一个 **Application**。Discord 会自动为每个新应用添加一个机器人用户；你可以在 **Bot** 选项卡中找到它。如果你在操作时遇到困难，Discord 的[设置指南](https://discordjs.guide/legacy/preparations/app-setup)中提供了截图。

从门户中复制两个值：

- **Bot** 选项卡中的 **token**：点击 **Reset Token** 生成它（Discord 只会显示一次）。它是你的代码用于登录的密码，因此请像对待密码一样保护它，不要向他人透露
- **General Information** 选项卡中的 **Application ID**。Discord 使用它将你的命令关联到此应用
